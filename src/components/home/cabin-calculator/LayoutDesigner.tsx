...
/** Read-only "everything together" architectural floor plan, shown right after the
 *  Add-ons step. Renders the proper dimensioned ModulePlan — walls with length × width,
 *  windows/doors on their walls, ceiling lights & fans, plug points, lifting hooks AND
 *  furniture (desks, cupboards, conference table) drawn to scale with measurements — plus
 *  a counted legend of everything chosen. Non-interactive; the LayoutDesigner below it is
 *  where the customer can drag items around. */
    ...
    <div className="rounded-xl border border-border bg-background p-3">
      <div className="mb-2 flex flex-wrap items-center gap-x-2 gap-y-0.5">
        ...
        <span className="text-[11px] text-muted-foreground">
          Everything you&rsquo;ve chosen — doors, windows, lights, fans, plug points &amp; furniture — to scale on one dimensioned floor plan{items.length > 0 ? ` (${items.length} items)` : ""}
        </span>
      </div>

      {/* Proper architectural 2D plan (dimensioned walls + furniture with sizes) */}
