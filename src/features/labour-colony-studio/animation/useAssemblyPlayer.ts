...
/**
 * LABOUR COLONY ASSEMBLY ANIMATION — playback state hook (spec: "Playback controls").
 *
 * Owns the transport state (playing / speed / loop) and the read-out state (current time, active step,
 * progress) the controls, overlay and step list render from. The animation CLOCK itself lives inside
 * the R3F render loop (AssemblyScene's useFrame — rAF-driven) for smoothness and determinism; the
 * scene reports its time back up through `reportTick`, and this hook pushes seek / step-jump commands
 * DOWN through a monotonic `seek` signal. Keeping the clock in the scene and the transport here means
 * the ~20 fps read-out never re-renders the WebGL subtree (AssemblyScene is memoised on stable props).
 */
...
export interface AssemblyPlayer {
  ...
  toggleLoop: () => void;
  /** called by the scene each animated frame (throttled internally before it hits state). */
