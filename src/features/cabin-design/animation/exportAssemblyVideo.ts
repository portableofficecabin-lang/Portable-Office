...
/**
 * ANIMATED CABIN ASSEMBLY — exporter (spec: "Video export" + "Export safety").
 *
 ...
 *
 *   • WebM  — MediaRecorder over a manually-driven canvas capture stream. Frames are rendered at a
 *             DETERMINISTIC timeline time (frameIndex / fps) — never a wall-clock delta — composited
 *             (WebGL frame → 2D canvas → baked captions) and pushed with requestFrame(). Genuinely
 *             supported in Chromium/Firefox; feature-detected before use.
 *   • PNG   — a store-only ZIP of the frame sequence (no new dependency), or a single hero/step frame.
 *   • MP4   — NOT produced in-browser here; the UI labels it unavailable rather than mis-naming a WebM.
 *
 * Determinism: identical config + fps + resolution ⇒ identical frame CONTENT. The exporter drives the
 * shared imperative apply path (AssemblyExportController.renderAt) so exported frames match the model
 * exactly. On completion / cancel / error it always restores the interactive renderer and stops the
 * capture stream.
 */

import { sampleCaption } from "./assemblyMotion";
import { drawAssemblyOverlay } from "./assemblyOverlayDraw";
...
const slug = (s: string): string => s.replace(/[^a-z0-9]+/gi, "-").replace(/^-+|-+$/g, "").toLowerCase() || "cabin";
...
/** Composite one deterministic frame: WebGL render → 2D canvas → baked caption overlay. `bgFill`, when
 *  set, paints an opaque backdrop first (WebM can't carry alpha, so a transparent scene would else go
 *  black). `keepCamera` renders the current on-screen camera (single "current frame" PNG). */
function composeFrame(
  controller: AssemblyExportController, ctx: CanvasRenderingContext2D, timeline: AssemblyTimeline,
  w: number, h: number, timeMs: number, drawOverlay: boolean, bgFill?: string, keepCamera?: boolean,
): void {
  const gl = controller.renderAt(timeMs, keepCamera);
  ctx.clearRect(0, 0, w, h);
  if (bgFill) { ctx.fillStyle = bgFill; ctx.fillRect(0, 0, w, h); }
  ctx.drawImage(gl, 0, 0, w, h);
  if (drawOverlay) drawAssemblyOverlay(ctx, w, h, sampleCaption(timeline, timeMs));
}
...
export async function runAssemblyExport(args: ExportRunArgs): Promise<ExportResult> {
  ...
  try {
    if (format === "png-frame") {
      ...
      composeFrame(controller, ctx, timeline, w, h, timeMs, drawOverlay, undefined, settings.keepCamera);
    ...
    if (format === "png-sequence") {
      ...
      for (let f = 0; f < totalFrames; f++) {
        ...
        composeFrame(controller, ctx, timeline, w, h, timeMs, drawOverlay);
    ...
    // ---- WebM ----
    if (!isWebMSupported()) throw new Error("In-browser WebM recording is not supported in this browser. Use the PNG frame sequence instead.");
    const mimeType = WEBM_CANDIDATES.find((t) => MediaRecorder.isTypeSupported(t))!;
    const totalFrames = Math.max(1, Math.ceil((timeline.totalMs / 1000) * fps));
    ...
    const cleanupStream = () => { try { track.stop(); } catch { /* ignore */ } stream.getTracks().forEach((t) => { try { t.stop(); } catch { /* ignore */ } }); };
    ...
    try {
      ...
      for (let f = 0; f < totalFrames; f++) {
        ...
        const timeMs = Math.min(timeline.totalMs, (f / fps) * 1000);
        composeFrame(controller, ctx, timeline, w, h, timeMs, drawOverlay, webmBgFill);
        track.requestFrame();
...
/* ----------------------------------------------------------------- store-only ZIP -------------- */
/* A minimal, dependency-free ZIP writer (STORE method — PNGs are already compressed). Emits local
 * file headers + central directory + EOCD; good for the frame-sequence download. */
