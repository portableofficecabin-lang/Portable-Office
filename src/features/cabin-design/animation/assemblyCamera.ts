/**
 * ANIMATED CABIN ASSEMBLY — camera-shot planner (spec: "Camera system").
 *
 * An original cinematic camera rig computed ENTIRELY from the model bounding box and the current
 * step's component group — no coordinates are hard-coded to a 20×10 cabin. Each construction step
 * gets a shot (establishing / low base / wall side / elevated roof / interior / opening close-up /
 * hero / orbit); the position + look-at target are framed from the active group so the camera always
 * shows the work being installed, and the final orbit / hero keep the completed cabin centred for any
 * cabin size.
 *
 * Pure math in THREE space (metres, Y-up) — the same mapping the interactive viewer uses
 * (viewer3d/partGeometry.ts). No three.js, no React, no DOM.
 */

import type { AssemblyStep, CabinPart, ModelBounds } from "@/features/cabin-design/model/types";
import { boxOfSolid, quadOfSolid, sceneCtxOf, toScene, type SceneCtx } from "@/features/cabin-design/viewer3d/partGeometry";
...
/** The whole model's three-space box, from its cabin-mm bounds. */
...
function partAABB(part: CabinPart, ctx: SceneCtx): { min: Vec3T; max: Vec3T } | null {
...
export function groupBoxOf(parts: CabinPart[], ctx: SceneCtx): Box3 | null {
  const min: Vec3T = [Infinity, Infinity, Infinity];
  const max: Vec3T = [-Infinity, -Infinity, -Infinity];
  let any = false;
...
export function sceneCtxForModel(model: CabinModel): SceneCtx {
...
const SHOTS: Record<CameraShotKind, ShotRecipe> = {
  establish: { dir: [1.0, 0.62, 1.1], distFactor: 2.3, tightness: 0.0, targetLift: 0.15 },
  "base-low": { dir: [0.9, 0.3, 1.15], distFactor: 2.1, tightness: 0.15, targetLift: -0.1 },
  "wall-side": { dir: [0.35, 0.5, 1.5], distFactor: 2.2, tightness: 0.1, targetLift: 0.1 },
  "roof-elevated": { dir: [0.8, 1.5, 0.85], distFactor: 2.2, tightness: 0.1, targetLift: 0.25 },
  interior: { dir: [1.2, 0.72, 0.55], distFactor: 1.6, tightness: 0.45, targetLift: 0.05 },
  "opening-closeup": { dir: [0.55, 0.42, 1.3], distFactor: 1.15, tightness: 0.75, targetLift: 0.0 },
  orbit: { dir: [1.05, 0.6, 1.2], distFactor: 2.35, tightness: 0.0, targetLift: 0.15 },
  hero: { dir: [1.1, 0.55, 1.25], distFactor: 2.3, tightness: 0.0, targetLift: 0.12 },
};

/** Which shot each construction step uses. */
export function shotForStep(step: AssemblyStep): CameraShotKind {
  switch (step) {
    case 1: case 2: return "base-low";
    case 3: return "establish";
    case 4: case 5: case 6: return "wall-side";
    case 7: case 8: case 9: return "interior";
    case 10: return "opening-closeup";
    case 11: case 12: return "roof-elevated";
    case 13: case 14: case 15: case 16: return "interior";
    case 17: default: return "hero";
  }
}
 ...
 * is framing. Guarantees a finite, sane pose for any cabin size.
 */
