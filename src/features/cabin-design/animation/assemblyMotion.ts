/**
 * ANIMATED CABIN ASSEMBLY — deterministic sampling (spec: "Animation behaviour" + "Determinism").
 *
...
import type {
  ...
  PartScheduleEntry, SceneSample, Vec3T,
} from "./assemblyTypes";
...
export function sampleCamera(timeline: AssemblyTimeline, timeMs: number): CameraKeyframe {
  ...
  // completed cabin, then settle into the hero three-quarter.
  if (timeMs >= lastEnd) {
...
export function samplePart(entry: PartScheduleEntry, timeMs: number, ctx: PartSampleCtx): PartRenderState {
  ...
  // camera can see the interior work. The current step's own envelope subject (insulation / internal
  // lining / ceiling) must NOT fade itself out, so it is excluded here and highlighted below.
...
export function sampleCaption(timeline: AssemblyTimeline, timeMs: number, activeStepIndex?: number): CaptionState {
  ...
  const rows = eng && options.showEngineeringCaptions ? step.engineering : [];
  return {
    ...base,
    kind: "step",
