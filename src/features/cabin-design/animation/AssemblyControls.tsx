...
/**
 * ANIMATED CABIN ASSEMBLY — playback + presentation controls (spec: "Playback controls" + "UX").
 *
 * Every control the spec calls for: play / pause / restart / prev-step / next-step, a scrub timeline,
 * speed (0.5–2×), loop, auto-camera, labels, dimensions, customer⇄engineering mode, background,
 * preview quality, fullscreen and Export. All buttons carry text labels / titles for accessibility
 * and are disabled while an export is running so nothing conflicts. Pure presentational — the parent
 * (AssemblyVideoView) owns the state.
 */

import {
  Camera, CameraOff, Captions, CaptionsOff, Download, Expand, Maximize2, Pause, Play, Repeat,
  RotateCcw, Ruler, SkipBack, SkipForward, Users,
} from "lucide-react";
...
function Toggle({ on, onClick, icon: Icon, label, disabled }: {
  on: boolean; onClick: () => void; icon: React.ComponentType<{ className?: string }>; label: string; disabled?: boolean;
}) {
  return (
    <button
...
export function AssemblyControls(props: AssemblyControlsProps) {
  const { player, totalMs, options, onOption, exporting } = props;
  const d = exporting;
  ...
  return (
    <div className="space-y-2 rounded-xl border border-border bg-muted/30 p-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-1 text-xs text-muted-foreground">
          ...
          {PLAYBACK_SPEEDS.map((s) => (
            <button key={s} type="button" onClick={() => player.setSpeed(s)} disabled={d}
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        <Toggle on={options.mode === "customer"} onClick={() => onOption({ mode: options.mode === "customer" ? "engineering" : "customer" })}
          icon={options.mode === "customer" ? Users : Ruler} label={options.mode === "customer" ? "Customer" : "Engineering"} disabled={d} />
        <Toggle on={options.autoCamera} onClick={() => onOption({ autoCamera: !options.autoCamera })} icon={options.autoCamera ? Camera : CameraOff} label="Auto camera" disabled={d} />
        <Toggle on={options.showLabels} onClick={() => onOption({ showLabels: !options.showLabels })} icon={options.showLabels ? Captions : CaptionsOff} label="Labels" disabled={d} />
        <Toggle on={options.showDimensions} onClick={() => onOption({ showDimensions: !options.showDimensions })} icon={Ruler} label="Dimensions" disabled={d} />
        ...
        <label className="ml-1 inline-flex items-center gap-1 text-xs text-muted-foreground">
          Background
          <select value={options.background} onChange={(e) => onOption({ background: e.target.value as AssemblyOptions["background"] })} disabled={d}
        ...
        <label className="inline-flex items-center gap-1 text-xs text-muted-foreground">
          Quality
          <select value={props.quality} onChange={(e) => props.onQuality(e.target.value as "low" | "high")} disabled={d}
            className="rounded-md border border-border bg-background px-1.5 py-1 text-xs text-foreground disabled:opacity-50">
