...
/**
 * ANIMATED CABIN ASSEMBLY — the R3F scene (spec: "Animation behaviour" + "Visual style").
 *
 * Renders the shared CabinModel ONCE (every part is a stable mesh, reusing the interactive viewer's
 * geometry mapping) and drives the animation IMPERATIVELY inside the render loop: each frame it reads
 * the current timeline time, samples the deterministic motion (per-part fly-in offset, opacity,
 * highlight, envelope cut-away) and the cinematic camera, and writes them straight onto the meshes +
 * camera — no per-frame React reconciliation, no geometry rebuild. The same apply path backs a
 * deterministic export controller (renderAt(ms) → the exporter composites + captions the frame).
 *
 * Reuses PartMesh's coordinate mapping (viewer3d/partGeometry) so a part sits in exactly the same
 * place here as in the 2D drawings and the 3D viewer. Mounted only behind the admin lazy island.
 */
...
import { boxOfSolid, quadOfSolid, sceneCtxOf, type SceneCtx } from "@/features/cabin-design/viewer3d/partGeometry";
...
const BG_HEX: Record<AssemblyBackground, string | null> = {
  studio: "#eef2f6", white: "#ffffff", site: "#e2e5e1", transparent: null,
};
const GROUND_HEX: Record<AssemblyBackground, string | null> = {
  studio: "#e2e8f0", white: "#f1f5f9", site: "#cfd6cb", transparent: null,
};

interface MeshRec { mesh: THREE.Mesh; base: Vec3T; material: THREE.MeshStandardMaterial; part: CabinPart; }
interface RenderItem { part: CabinPart; base: Vec3T; size: Vec3T | null; geo: THREE.BufferGeometry | null; }

function buildRenderList(model: CabinModel, ctx: SceneCtx): RenderItem[] {
  return model.parts.map((part) => {
    const b = boxOfSolid(part.solid, ctx);
    if (b) return { part, base: b.center, size: b.size, geo: null };
    ...
    if (q) {
      ...
      return { part, base: [0, 0, 0], size: null, geo: g };
    }
    return { part, base: [0, 0, 0], size: [0.01, 0.01, 0.01], geo: null };
  });
}

/** Pull a camera pose back from its target when the frame is narrower than 16:9, so a wide cabin
 *  fits horizontally in portrait/square exports (the shot distance is tuned for a fixed vertical FOV). */
function fitForAspect(k: { position: Vec3T; target: Vec3T }, aspect: number): Vec3T {
  const REF = 16 / 9;
  if (aspect >= REF) return k.position;
  // horizontal visible extent scales LINEARLY with camera distance, so to keep a wide cabin the same
  // fraction of frame width at a narrower aspect the distance must scale by REF/aspect (not its sqrt).
  // Over-pulling in portrait only adds harmless vertical margin, so this is the safe minimal fit.
...
function SceneContents(props: AssemblySceneProps) {
  ...
  // timeline rebuild) so a BOQ rate change / caption toggle never invalidates renderList and remounts
  // every mesh. It is value-equal to timeline.sceneCtx (both come from the same model bounds).
  ...
  /* ---- the single apply function (shared by the loop AND the exporter) ---- */
  const applyFrame = useCallback((timeMs: number, opts?: { forceAutoCamera?: boolean; ignoreSelection?: boolean; keepCamera?: boolean }) => {
    const tl = timelineRef.current;
    const stepIndex = activeStepIndexAt(tl, timeMs);
    const cutaway = activeStepCutawayAt(tl, timeMs);
    const partCtx = { activeStepIndex: stepIndex, activeStepCutaway: cutaway, stepCount: tl.steps.length, options: tl.options };
    ...
    for (const entry of tl.schedule) {
      ...
      if (!visible) continue;
      rec.mesh.position.set(rec.base[0] + st.offset[0], rec.base[1] + st.offset[1], rec.base[2] + st.offset[2]);
      ...
      if (isSel) { mat.color.set("#f59e0b"); mat.emissive.set("#7c3a00"); mat.emissiveIntensity = 0.6; }
      else if (st.highlight) { mat.color.set(rec.part.colorHex); mat.emissive.set("#b45309"); mat.emissiveIntensity = 0.5; }
      else { mat.color.set(rec.part.colorHex); mat.emissive.set("#000000"); mat.emissiveIntensity = 0; }
    }
    ...
    if (auto) {
      ...
      // pull the camera back for narrow (portrait/square) aspect ratios so a wide cabin never crops —
      // the framing is computed for a fixed vertical FOV, so a sub-16:9 frame needs more distance.
  ...
  /* ---- the interactive loop (advances only while playing; export bypasses it) ---- */
  ...
  /* ---- re-apply the paused frame when playback stops or a presentation prop changes.
   *      Critically this re-enables OrbitControls when the animation pauses/ends (the last playing
   *      frame left them disabled while the camera was auto-driven) — spec: manual orbit while paused. */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const controller: AssemblyExportController = {
      ...
      setSize: (w, h) => {
        exportSaved.current = { size: gl.getSize(new THREE.Vector2()), pr: gl.getPixelRatio(), aspect: (camera as THREE.PerspectiveCamera).aspect };
        gl.setPixelRatio(1);
  ...
  const handleSelect = useCallback((id: string) => (e: ThreeEvent<MouseEvent>) => { e.stopPropagation(); onSelectProp(id); }, [onSelectProp]);
...
export const AssemblyScene = memo(function AssemblyScene(props: AssemblySceneProps) {
  const radius = props.timeline.radius;
