...
// True when a section has anything worth rendering (parts always render).
function sectionHasContent(s: SpecSection): boolean {
  if (s.kind === "part") return true;
  return Boolean(
    (s.title || "").trim() ||
      s.rows.some((r) => r.label.trim() || r.value.trim()) ||
      (s.blocks || []).some((b) => b.heading.trim() || b.bullets.some((x) => x.trim())),
  );
}

// Display number for one item: explicit number wins; empty string = no number;
// undefined (legacy) falls back to the supplied auto-number.
function displayPrefixCore(section: SpecSection, autoNum: number): string {
...
// Precompute id -> prefix so auto-numbering counts only rendered item sections
// (never part dividers or skipped empties). Used identically by PDF + preview.
function buildPrefixMap(sections: SpecSection[]): Record<string, string> {
  const map: Record<string, string> = {};
  let auto = 0;
  for (const s of sections) {
    if (s.kind === "part" || !sectionHasContent(s)) {
      map[s.id] = "";
      continue;
    }
    auto += 1;
    map[s.id] = displayPrefixCore(s, auto);
  }
  return map;
}
...
export default function AdminSpecifications() {
  ...
  // Start blank to avoid an SSR/hydration mismatch around the UTC date boundary;
  // today's date is filled in on mount (see effect below).
  ...
  // Fill today's date on the client only (avoids hydration mismatch).
  useEffect(() => {
    setDocDate((d) => d || new Date().toISOString().slice(0, 10));
  }, []);
  ...
  const generatePdf = async () => {
    try {
      ...
      // Reserve the bottom band for the page footer (~277-281mm) AND the 2-line
      // IT-Act legal footer added by addLegalFooter (~289-292mm) so nothing collides.
      ...
      const drawFooter = (pageNum: number, total: number) => {
        // Sits above the 2-line IT-Act legal footer (first baseline ~288.8mm).
        const fy = pageH - 20;
        doc.setDrawColor(...BRAND_DARK);
        doc.setLineWidth(0.4);
      ...
      const drawItemBar = (text: string) => {
        ensure(16); // bar (9.5) + first row/heading so the title is never orphaned
      ...
      const drawBlockHeading = (heading: string) => {
        ...
        ensure(6.5 + 4.2); // heading + first bullet line, so the heading isn't orphaned
        doc.setFont("helvetica", "bold");
        doc.setFontSize(9.8);
        doc.setTextColor(...BRAND_DARK);
      ...
      sections.forEach((section) => {
        ...
        drawItemBar(`${prefixMap[section.id] || ""}${section.title || "Untitled"}`);
      ...
      // Signatory block — reserve the approval stamp too (~22mm) when set, so the
      // signature + stamp break to a new page together instead of the stamp orphaning.
      ensure(approver ? 84 : 62);
      y += 3;
      doc.setFont("helvetica", "italic");
      doc.setFontSize(8.5);
      doc.setTextColor(...TEXT_GRAY);
      ...
      // Approval stamp (space already reserved with the signatory block above).
      if (approver) {
        const sw = 75;
        const sh = 16;
        const sx = rightX - sw;
  ...
  const prefixMap = buildPrefixMap(sections); // item numbering for the live preview
  ...
  return (
    ...
    >
      ...
      <div className={`${showPreview ? "block" : "hidden"} print:block`}>
        {/* Print sizing/colour: A4 with no browser margins, keep the coloured bars + watermark. */}
        <style>{`@media print {
          body { background: #fff !important; }
          #spec-print { box-shadow: none !important; margin: 0 !important; }
